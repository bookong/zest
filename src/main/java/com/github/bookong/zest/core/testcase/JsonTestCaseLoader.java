...
public class JsonTestCaseLoader  {
	...
	private void loadTestParam(Object parent, String fieldName, JSONObject paramValue) {
		try {
			Field field = ZestReflectHelper.getFieldByFieldName(parent, fieldName);
			if (field == null) {
			...
			if (fieldValue == null || JSONUtils.isNull(fieldValue)) {
				ZestReflectHelper.setValueByFieldName(parent, fieldName, null);
				return;
			}

			Class<?> type = field.getType();
			if (type.isAssignableFrom(Long.class) || "long".equals(type.getName())) {
				ZestReflectHelper.setValueByFieldName(parent, fieldName, paramValue.getLong(fieldName));

			} else if (type.isAssignableFrom(Integer.class) || "int".equals(type.getName())) {
				ZestReflectHelper.setValueByFieldName(parent, fieldName, paramValue.getInt(fieldName));

			} else if (type.isAssignableFrom(Boolean.class) || "boolean".equals(type.getName())) {
				ZestReflectHelper.setValueByFieldName(parent, fieldName, paramValue.getBoolean(fieldName));

			} else if (type.isAssignableFrom(Double.class) || "double".equals(type.getName())) {
				ZestReflectHelper.setValueByFieldName(parent, fieldName, paramValue.getDouble(fieldName));

			} else if (type.isAssignableFrom(Float.class) || "float".equals(type.getName())) {
				ZestReflectHelper.setValueByFieldName(parent, fieldName, 
						Double.valueOf(paramValue.getDouble(fieldName)).floatValue());

			} else if (type.isAssignableFrom(String.class)) {
				ZestReflectHelper.setValueByFieldName(parent, fieldName, paramValue.getString(fieldName));
				...
				Object memberClass = ZestReflectHelper.getValueByFieldName(parent, fieldName);
				if (memberClass == null) {
					memberClass = type.newInstance();
					ZestReflectHelper.setValueByFieldName(parent, fieldName, memberClass);
				}
	...
	private void loadTestParamList(Object parent, String fieldName, JSONObject paramValue, Field field) {
		try {
			List list = (List)ZestReflectHelper.getValueByFieldName(parent, fieldName);
			if (list == null) {
				list = new ArrayList();
				ZestReflectHelper.setValueByFieldName(parent, fieldName, list);
			}
	...
	private void loadTestParamMap(Object parent, String fieldName, JSONObject paramValue, Field field) {
		try {
			Map map = (Map)ZestReflectHelper.getValueByFieldName(parent, fieldName);
			if (map == null) {
				map = new HashMap();
				ZestReflectHelper.setValueByFieldName(parent, fieldName, map);
			}
