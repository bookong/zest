...
public class DbUnitExcuter extends AbstractJdbcExcuter {
        ...
        try {
            ...
            for (Entry<String, TargetTable> entry : db.getTargetTables().entrySet()) {
                if (entry.getValue().isIgnoreTargetTableVerify()) {
                    System.out.println("DB: \"" + db.getDatabaseName() + "\", table \"" + entry.getKey() + "\" ignore verify.");
                } else {
    ...
    /** 验证每行的数据 */
    private void verifyRowData(String baseMessage, TestCaseData testCaseData, Map<String, Object> expectedRowDatas, Map<String, Object> actualRowDatas) {
        ...
        for (Entry<String, Object> item : expectedRowDatas.entrySet()) {
            ...
            // 只验证 expected 中指定的列
            Assert.assertTrue(baseMessage + "must has column \"" + expectedColName + "\"", actualRowDatas.containsKey(expectedColName));
            ...
            if (expectedColData == null) {
                ...
                Assert.assertEquals(baseMessage, ZestDateUtils.formatDateNormal((Date) expectedColData), ZestDateUtils.getStringFromDBDate((Date) actualColData, testCaseData));
            } else {
                // 具体值验证 - 非日期的其他类型
